...
local function complete_with_tools(options, messages, tool_names)
    ...
    while true do
        ...
        if next_prompt ~= "" then
            ...
            if #tool_names > 0 then
                response = complete_with_tools(options, messages, tool_names)
            else
...
return yaaf.command({
    ...
    options = {
        ...
        },
        {
            ...
            flags = { "--stream" },
            type = "flag",
            description = "Stream model output incrementally for chat",
        },
        {
            name = "think",
            flags = { "--think" },
