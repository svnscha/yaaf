...
[[nodiscard]] ScriptAgentContext &context(lua_State *state)
{
    return *static_cast<ScriptAgentContext *>(lua_touserdata(state, lua_upvalueindex(1)));
}
...
[[nodiscard]] yaaf::llm::ChatResponse run_chat(ScriptAgentContext &runtime, std::string endpoint,
...
void push_agent_function(lua_State *state, ScriptAgentContext &runtime, lua_CFunction function)
{
...
int lua_emit(lua_State *state)
{
    try
    {
        ...
        {
            write_text_event(*runtime.output, "yaaf", content);
        }
