...
Yaaf loads MCP tools only when a config file path is supplied. Use `--mcp <path>` with `ask`, `chat`, `agent`, or `run`, or set `YAAF_MCP_FILE` in the process environment or `.env` file. Legacy direct-script compatibility with `yaaf --mcp <path> <file.lua>` still works.
